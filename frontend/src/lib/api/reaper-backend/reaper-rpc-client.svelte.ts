...
export class ReaperRpcClient {
	...
	public async isBackgroundProjectsEnabled(): Promise<boolean> { 
		const commands = [ 
			new SetOperationCommand("isBackgroundProjectsEnabled"), 
			new RunScriptCommand(), 
			new GetStateCommand(SectionKeys.ReaperSetlist, "enabled"), 
		] as const; 
		const [_setOperation, _runScript, enabledRaw] = await this.apiClient.executeCommands(commands);  
		if (enabledRaw === undefined) { 
			throw new Error("Failed to retrieve enabled. Please check the script configuration."); 
		} 
